...
function Customers() {
    ...
    const loadCustomers = async (search = '') => {
        try {
            ...
            setError(null);
            let data;
            if (search) { data = await customerService.search(search); }
            else { data = await customerService.getAll(); }
            setCustomers(data);
    ...
    const resetForm = () => {
        setFullName(''); setDocument(''); setPhone(''); setAddress('');
    ...
    const handleSubmit = async (e) => {
        ...
        } catch (err) { alert('Error: ' + (err.response?.data?.message || 'No se pudo guardar')); }
        ...
        <div style={container(isMobile)}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobile ? '12px' : '18px', flexWrap: 'wrap', gap: '8px' }}>
                ...
                <button onClick={() => { resetForm(); setShowForm(!showForm); }}
                        style={btnPrimary(isMobile, showForm)}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ marginBottom: '10px' }}>
                            ...
                            <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required
                                   placeholder="Ej: Juan Perez" style={inputStyle(isMobile)} autoFocus />
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                            <div>
                                ...
                                <input type="text" value={document} onChange={(e) => setDocument(e.target.value)}
                                       placeholder="Ej: 12345678" style={inputStyle(isMobile)} />
                            </div>
                            <div>
                                <label style={labelStyle}>Telefono</label>
                                <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)}
                                       placeholder="Ej: 3001234567" style={inputStyle(isMobile)} />
                            </div>
                        </div>
                        <div style={{ marginBottom: '14px' }}>
                            <label style={labelStyle}>Direccion</label>
                            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)}
                                   placeholder="Ej: Calle 123 #45-67" style={inputStyle(isMobile)} />
                        </div>
                        <div style={{ display: 'flex', gap: '8px', flexDirection: isMobile ? 'column' : 'row' }}>
                            <button type="submit" disabled={saving}
                                    style={btnSave(isMobile, saving)}>
                                {saving ? 'Guardando...' : editingCustomer ? 'Actualizar' : 'Guardar'}
                            </button>
                            <button type="button" onClick={resetForm} style={btnCancel(isMobile)}>
                                Cancelar
                            </button>
                        </div>
                    </form>
            ...
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' }}>
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                       placeholder="🔍 Buscar por nombre..." style={{ ...inputStyle(isMobile), flex: 1, minWidth: '150px' }} />
                <button type="submit" style={btnSmall('#1a1a1a', 'white')}>Buscar</button>
                {searchTerm && (
                    <button type="button" onClick={() => { setSearchTerm(''); loadCustomers(); }} style={btnSmall('white', '#666')}>Limpiar</button>
                )}
            </form>
            ...
            {isMobile ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {customers.length === 0 ? (
                        <div style={emptyBox}>No hay clientes</div>
                    ) : (
                            ...
                            <div key={customer.id} style={cardStyle}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    ...
                                    <span style={{
                                        ...
                                        backgroundColor: customer.status === 'ACTIVE' ? '#F0FFF4' : '#FFF5F5',
                                        color: customer.status === 'ACTIVE' ? '#00A650' : '#E31837',
                                    }}>
                    {customer.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}
                                ...
                                <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                                    ...
                                    {customer.status === 'ACTIVE' ? (
                                        <button onClick={() => handleDeactivate(customer.id, customer.fullName)} style={btnCard('#999')}>Desactivar</button>
                                    ) : (
                                        <button onClick={() => handleActivate(customer.id)} style={btnCard('#00A650')}>Activar</button>
                                    )}
                                </div>
                            </div>
                        ))
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '12px', overflow: 'hidden', border: '1px solid #EEEEEE' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                        ...
                        <tbody>
                        {customers.length === 0 ? (
                            <tr><td colSpan="5" style={{ padding: '30px', textAlign: 'center', color: '#999' }}>No hay clientes</td></tr>
                        ) : (
                            customers.map(customer => (
                      ...
                      <span style={{
                        ...
                        {customer.status === 'ACTIVE' ? 'Activo' : 'Inactivo'}
                                    ...
                                    <td style={{ ...tdStyle, textAlign: 'right' }}>
                                        ...
                                        {customer.status === 'ACTIVE' ? (
                                            <button onClick={() => handleDeactivate(customer.id, customer.fullName)} style={btnTable('#999')}>Desactivar</button>
                                        ) : (
                                            <button onClick={() => handleActivate(customer.id)} style={btnTable('#00A650')}>Activar</button>
                                        )}
                                    </td>
                                </tr>
                            ))
...
const formBox = (m) => ({
    backgroundColor: 'white', borderRadius: '12px', padding: m ? '16px' : '20px',
    marginBottom: '14px', border: '1px solid #EEEEEE', animation: 'fadeInUp 0.3s ease',
    maxWidth: '550px', margin: '0 auto 14px',
});
